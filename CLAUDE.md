...
`/ui/native` is split per-RID: `/ui/native/win-arm64/` and `/ui/native/win-x64/`, matching `ui-build-pipeline.md` §7.1's `RuntimeIdentifiers` list. `LimelightX.UI.csproj` resolves and copies whichever folder matches the current build/publish RID (falling back to the host machine's own OS architecture when no RID is pinned) automatically. Both DLLs are currently built for **ARM64 only** — `/ui/native/win-arm64/` is populated, `/ui/native/win-x64/` is not yet (the layout is decided; building the actual x64 binary, per `spec/parsing/tree-sitter-build-guide.md` §9, is explicitly deferred future work). Until `/ui/native/win-x64/` is populated, any test or code path that loads either DLL must be skippable/gated on the `windows-latest` (x64) CI runner — done via the `NativeTreeSitter` xUnit trait and `ui-ci.yml`'s `--filter "Category!=NativeTreeSitter"`. This entry governs `/ui/native`, `/ui/queries`, and `/ui/intellisense` (see §1) and does not license any further native/interop additions without explicit approval.
