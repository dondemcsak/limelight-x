...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_tree_sitter_limelightx", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_tree_sitter_limelightx": ["*.pyi", "py.typed"],
        "tree_sitter_tree_sitter_limelightx.queries": ["*.scm"],
    },
    ext_package="tree_sitter_tree_sitter_limelightx",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_tree_sitter_limelightx/binding.c",
                "src/parser.c",
            ],
            define_macros=[
